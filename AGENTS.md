...
├── kb/                 # Knowledge bases (developer context, Trinity, Codex skills)
├── logs/               # Session logs
...
| Library | Purpose |
|---------|---------|
| `anthropic` / `Codex-agent-sdk` | Codex API & agent runner |
...
| 3 | **Anthropic Codex** (`Codex-*`) | Cloud, but no OpenAI dependency — for tasks needing strong reasoning |
...
- `DOME_PROVIDER=Codex` → all agents use Anthropic API
...
- `researcher` → Codex (needs web, cloud acceptable)
- `planner` → local or Codex depending on complexity
...
| Variable | Purpose |
|----------|---------|
| `DOME_PROVIDER` | `local` / `Codex` / `mixed` — controls agent provider strategy |
| `DOME_LOCAL_MODEL` | Default Ollama model (default: `llama3.1:8b`) |
| `ANTHROPIC_API_KEY` | Codex API — use `pass dome/anthropic-key` |
| `ANTHROPIC_MODEL` | Override Codex model (default: `Codex-opus-4-6`) |
| `CLAUDE_AGENT_WORKDIR` | Codex workspace root (default: `DOME-HUB/`) |
...
# Bootstrap Codex compute env
bash compute/bootstrap-Codex.sh

# Run Codex agent
python agents/Codex/runner.py --prompt "..."
