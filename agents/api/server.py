...
def _trace(agent: str, prompt: str, response: str, elapsed: float):
    _traces.append({"agent": agent, "prompt": prompt[:200], "response": response[:200], "elapsed": elapsed, "ts": time.time()})
    if len(_traces) > 200:
        _traces.pop(0)
...
async def get_memory(name: str):
    ...
    return {"agent": name, "memory": [{"role": m.role, "content": m.content} for m in agent.memory]}
...
async def health():
    ...
    stack = {
        ...
        "chroma":   next((c["status"] for c in checks if c["name"] == "chroma"), "unknown"),
        "ollama":   next((c["status"] for c in checks if c["name"] == "ollama"), "unknown"),
    }
