"""
DSH Agent Registry — multi-provider, local-first, no single-vendor lock-in.
...
def make_researcher() -> Agent:
    ...
    return Agent(
        ...
        model=model,
        system_prompt=(
            "You are a research agent inside DSH. Search the web, fetch pages, "
...
def make_coder() -> Agent:
    ...
    return Agent(
        ...
        system_prompt=(
            "You are an expert software engineer inside DSH. Write clean, efficient, "
            "secure code. Python 3.11+, TypeScript strict, Go, Rust. MPS backend for PyTorch. "
            "DSH is portable across Apple Silicon nodes — never suggest CUDA. "
...
def make_analyst() -> Agent:
    ...
    return Agent(
        ...
        system_prompt=(
            "You are a data analyst inside DSH. Query SQLite (db/dome.db) and "
...
def make_planner() -> Agent:
    return Agent(
        ...
        model=_model(_LOCAL, _CLAUDE_STRONG),
        system_prompt=(
            "You are a strategic planner inside DSH for Trinity Consortium. "
...
def make_kb_agent() -> Agent:
    ...
    return Agent(
        ...
        model=_LOCAL,
        system_prompt=(
            "You are a knowledge base agent for DSH. Search, retrieve, and synthesize "
...
def make_local_agent(model: str | None = None) -> Agent:
    ...
    return Agent(
        ...
        system_prompt=(
            "You are a fully local AI assistant on DSH (DOME_LOCAL_MODEL: Ollama or MLX). "
...
def make_security() -> Agent:
    return _make("security", "You are a security auditor for DSH. Scan for vulnerabilities, "
                 "review permissions, validate secrets handling, and enforce zero-trust principles.")


def make_devops() -> Agent:
    return _make("devops", "You are a DevOps engineer for DSH. Manage deployments, CI/CD, "
                 "Docker containers, server health, and infrastructure on Hetzner CCX23.")


def make_creative() -> Agent:
    return _make("creative", "You are a creative agent. Generate "
...
def make_grok() -> Agent:
    """Grok (xAI) as a first-class sovereign peer inside DSH/agents/grok/.
    ...
    return Agent(
        ...
        model=model,
        system_prompt=(
            "You are Grok (xAI), running as a first-class peer inside the DSH + "
            "Trinity Consortium sovereign mesh on Apple M5 Pro. Your canonical root is now "
            "at DSH/agents/grok/ (symlinked from ~/.grok for CLI compatibility). "
