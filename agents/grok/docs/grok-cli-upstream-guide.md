...
| `auth: external auth provider timed out after 60s` | Binary didn't exit within 60 seconds and was killed |
...
| `x-grok-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct inference cluster. \*Can be omitted for `grok-build` which is on the default IC, but always safe to include. |

**Streaming vs non-streaming:**

Most models behind the proxy are hosted on internal inference clusters that only support streaming. Always use `"stream": true` unless you know the model supports non-streaming.
...
> **Note:** `--tools`, `--disallowed-tools`, `--max-turns`, `--effort`, and `--permission-mode` are only supported in headless mode (`-p`). If used in the interactive TUI, a warning is printed and the flag is ignored.
...
Override telemetry destinations and credentials. Empty values disable the corresponding sink. Env vars take precedence over config values.

```toml
[telemetry]
events_url = "https://grok.com/_data/v1/events"  # env: GROK_TELEMETRY_EVENTS_URL
...
Force all subagents to use a single model regardless of agent definition or parent session:
...
This takes absolute precedence over `[subagents.models]`, agent definition models, and parent inheritance.
...
Model overrides (`[subagents.models]`, agent definition `model`) only apply when the parent is on a heavy model. Otherwise subagents inherit the parent model. The global `default_model` bypasses this gate.
...
context_window = 128000               # Total context window in tokens (for auto-compact)
```
...
| `embedding.model` | `embedding-beta-3-small` | Embedding model for vector search |
...
| `GROK_LOG_FILE`                 | Enable file logging: `1` for `~/.grok/logs/tracing.log`, or a custom path                               |
| `GROK_LOG_FILTER`               | Log level filter when `GROK_LOG_FILE` is set (default: `info`)                                           |
| `RUST_LOG`                      | Logging level for stderr (headless mode only; TUI captures stderr)                                       |
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `GROK_LOG_FILE` instead:

```bash
# Log to the default file (~/.grok/logs/tracing.log)
GROK_LOG_FILE=1 grok

# Log to a custom path
GROK_LOG_FILE=/tmp/grok-debug.log grok

# Control the log level (default: info)
...
GROK_LOG_FILE=1 GROK_LOG_FILTER=debug grok -p "hello"
grep "auth:" ~/.grok/logs/tracing.log
```
