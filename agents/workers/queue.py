"""
DSH Task Queue — Redis-backed with SQLite persistence
"""
...
def _insert_task(
    task_id: str, agent: str, prompt: str, priority: int, callback_url: Optional[str]
):
    conn = _db()
    conn.execute(
        ...
        while True:
            ...
            try:
                ...
                result = await asyncio.get_event_loop().run_in_executor(
                    None,
                    orc.agents.get(task["agent"], next(iter(orc.agents.values()))).run,
                    task["prompt"],
                )
...
async def _fire_callback(
    url: str, task_id: str, result: Optional[str], error: Optional[str]
):
    try:
