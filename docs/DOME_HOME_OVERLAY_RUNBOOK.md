# DOME-HUB Home Overlay Runbook

## Canonical Roles

- `$HOME` remains the macOS account home and compatibility route.
- `$HOME/DOME-HUB` is the private canonical DOME control repo.
- `$HOME/DOME-HUB/home` is private runtime home data and is ignored by git.
...
Already moved into `DOME-HUB/home` and symlinked back:

- `$HOME/trinity-consortium`
- `$HOME/go`
- `$HOME/.agents`
- `$HOME/.codex`
- `$HOME/.config`
...
1. Stop active Trinity services under `projects/trinity-consortium` and `trinity-unified-ai`, then run:
...
Rollback uses the private manifest in `DOME-HUB/home/.dome-overlay-manifest.json`.
