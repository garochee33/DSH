...
- **What the export excludes** (see `config/public-export.denylist` and `PREFIX:` rules in `scripts/export-to-dsh.sh`):
  ...
  - **`docs/DOME-HUB-ARCHITECTURE.md`** (full internal architecture reference).
...
- **Production mesh peer** (HMAC, retries): use private `scripts/mycelium-signal.sh` when operating a bound node (vendored in full DOME-HUB; not required for Phase 1 reading).
