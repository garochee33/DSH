# Repo Flow: Private DSH -> Public DSH

## Canonical Roles

- `DSH` (`~/DSH`) is the private canonical repo.
- `DSH` (`~/DSH`) is the public sanitized distribution.
- Direction of truth is one-way by default: `DSH -> DSH`.
...
1. Work in `DSH` as private canonical.
