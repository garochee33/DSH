...
curl -X POST http://127.0.0.1:8001/voice/speak \
  -H 'Content-Type: application/json' \
  -d '{"text":"DOME-HUB voice pipeline online.","allow_cloud":true}'
```

## Env Knobs

```bash
DOME_VAD_MODEL=$HOME/DOME-HUB/models/asr/silero_vad.onnx
DOME_ASR_MODEL=base
DOME_ASR_BACKEND=auto
DOME_ASR_DEVICE=cpu
DOME_WHISPER_MODEL_DIR=$HOME/DOME-HUB/models/asr/whisper
