...
Concise rules for how Claude creates, reads, and delivers files inside DSH.
...
| `/sessions/<id>/mnt/DSH/` | The user's workspace folder. Persistent on the host. Final deliverables go here. |
...
- **< 100 lines** → write directly to `DSH/` in one call.
- **> 100 lines** → create an empty file in `DSH/`, then fill it with iterative `Edit`s.
...
```
[View your report](computer:///sessions/<id>/mnt/DSH/report.docx)
```
