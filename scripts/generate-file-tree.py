...
DOME_ROOT = Path(os.environ.get("DOME_ROOT", Path.home() / "DOME-HUB")).resolve()
...
def load_registry() -> dict[str, Path]:
    ...
    for line in REGISTRY.read_text(encoding="utf-8").splitlines():
        ...
        if m_path and current_name:
            ...
            current_name = None
    return repos or {"dome-hub": DOME_ROOT}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate FILE_TREE.md for DOME-HUB repos.")
    parser.add_argument("--repo", default="dome-hub", help="Registry name (fractalmap-repos.yaml)")
