#!/bin/bash
# DSH lock-down phase 4 — close the remaining gaps:
...
# Anchor file for DSH
PF_ANCHOR=/etc/pf.anchors/com.dsh
sudo tee "$PF_ANCHOR" >/dev/null <<'EOF'
# DSH pf anchor — drop outbound to known telemetry endpoints.
...
EOF
if ! grep -q "com.dsh" /etc/pf.conf 2>/dev/null; then
  sudo cp /etc/pf.conf /etc/pf.conf.pre-dome.bak
  echo 'anchor "com.dsh"' | sudo tee -a /etc/pf.conf >/dev/null
  echo 'load anchor "com.dsh" from "/etc/pf.anchors/com.dsh"' | sudo tee -a /etc/pf.conf >/dev/null
