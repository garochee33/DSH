...
REPOS=(
  "$DOME_ROOT"
  "$DOME_ROOT/home/projects/trinity-consortium"
  "$DOME_ROOT/home/projects/s3xyverse/s3xyverse-next"
  "$DOME_ROOT/home/projects/dome-console"
  "$DOME_ROOT/home/projects/paradise-estate-mykonos"
  "$DOME_ROOT/home/projects/alchemmical-cabaret-33"
)
...
if [ -f "$DOME_ROOT/tsconfig.json" ]; then
  ...
  if pnpm typecheck >> "$GATE_LOG" 2>&1; then
    ok "DOME-HUB: TypeScript clean"
  else
    fail "DOME-HUB: TypeScript errors"
  fi
fi

# Trinity Consortium
TC="$DOME_ROOT/home/projects/trinity-consortium"
if [ -f "$TC/package.json" ]; then
  if cd "$TC" && pnpm check >> "$GATE_LOG" 2>&1; then
    ok "trinity-consortium: TypeScript clean"
  else
    fail "trinity-consortium: TypeScript errors"
  fi
fi

# S3XYVERSE
SX="$DOME_ROOT/home/projects/s3xyverse/s3xyverse-next"
if [ -f "$SX/package.json" ]; then
  if cd "$SX" && npx tsc --noEmit >> "$GATE_LOG" 2>&1; then
    ok "s3xyverse-next: TypeScript clean"
  else
    fail "s3xyverse-next: TypeScript errors"
  fi
fi
...
if source .venv/bin/activate 2>/dev/null && python3 -c "from agents.core.rag import RAGPipeline; from agents.core.tools import ALL_TOOLS" 2>/dev/null; then
  ok "DOME-HUB: Python imports clean"
else
  fail "DOME-HUB: Python import errors"
fi
...
if [[ "$MODE" == "--pre-deploy" || "$MODE" == "--full" ]]; then
  ...
  if [ -f "$TC/package.json" ]; then
    if cd "$TC" && pnpm build >> "$GATE_LOG" 2>&1; then
      ok "trinity-consortium: build PASS"
    else
      fail "trinity-consortium: build FAILED"
    fi
  fi

  if [ -f "$SX/package.json" ]; then
    if cd "$SX" && pnpm build >> "$GATE_LOG" 2>&1; then
      ok "s3xyverse-next: build PASS"
    else
      fail "s3xyverse-next: build FAILED"
    fi
  fi
else
