#!/usr/bin/env python3
"""Sync canonical DOME-HUB skill docs into agent-specific SKILL.md mirrors.
...
TARGET_ROOTS = [
    ROOT / "agents" / "Codex" / "skills" / "dome-hub",
    ROOT / "home" / ".codex" / "skills" / "dome-hub",
    ROOT / "home" / ".unified-ai" / "skills" / "dome-hub",
]

DESCRIPTIONS = {
    "algorithms": "Algorithm design and analysis skill for DOME-HUB; use when working on optimization, data structures, or computational procedures.",
    ...
    "sacred_geometry": "Sacred geometry and E8/phi construction skill; use when working on geometric symbolism, lattice structure, or sacred-form generation.",
    "skill-creator": "Skill authoring and optimization skill; use when creating, improving, testing, or packaging skills for DOME-HUB agents.",
    "visual-storytelling": "Visual storytelling skill chain for DOME-HUB; use for narrative motion, scroll-driven UX, and stack-aware implementation rails.",
...
def sync_skill(skill: SkillSource) -> list[Path]:
    ...
    for root in TARGET_ROOTS:
        ...
        description = DESCRIPTIONS.get(
            skill.name, f"DOME-HUB skill for {skill.name}. Use when working in the `{skill.name}` domain."
        )
        wrapped = (
            f"---\n"
            f"name: {skill.name}\n"
...
def write_index(skills: list[SkillSource]) -> None:
    ...
    for root in TARGET_ROOTS:
        ...
        if root.name == "dome-hub" and root.parent.name == "skills":
