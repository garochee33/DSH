#!/usr/bin/env bash
# DOME-HUB: structural map auto-update — fractalmap, FILE_TREE.md, holographic tree map.
...
if [[ "$NEEDS_REFRESH" == "1" ]] && [[ -x "$DOME_ROOT/scripts/fractalmap-generate.sh" ]]; then
  if bash "$DOME_ROOT/scripts/fractalmap-generate.sh" dome-hub >>"$LOG" 2>&1; then
    log_msg "fractalmap dome-hub OK (agent=$AGENT sha=${CURRENT_SHA:0:8})"
  else
    log_msg "fractalmap dome-hub FAILED (agent=$AGENT)"
  fi
fi
...
if [[ "$NEEDS_REFRESH" == "1" ]] && [[ -f "$DOME_ROOT/scripts/generate-file-tree.py" ]]; then
  if python3 "$DOME_ROOT/scripts/generate-file-tree.py" --repo dome-hub >>"$LOG" 2>&1; then
