...
#   Hot   → ~/DSH/archive/dsh/brain-vault/<date>/
#   Always→ ~/iCloud/DOME-ARCHIVE/dsh/brain-vault/<date>/
#
...
# ---- paths -------------------------------------------------------------------
VAULT="${HOME}/DSH/brain/vault"
HOT_ROOT="${HOME}/DSH/archive/dsh/brain-vault"
ICLOUD_ROOT="${HOME}/iCloud/DOME-ARCHIVE/dsh/brain-vault"
PENDING_MARKER="${HOME}/DSH/logs/vault-snapshot.icloud-pending"
...
if [[ "${ICLOUD_AVAILABLE}" == "1" ]]; then
  if rsync_to "${ICLOUD_DIR}" 2>/dev/null; then
    ...
    echo "[vault-snapshot] WARN: writing pending marker — run from a Terminal session to catch up." >&2
    {
      ...
      echo "remediation: run 'bash ~/DSH/scripts/vault-snapshot.sh' from a Terminal (TCC-inherited)"
    } > "${PENDING_MARKER}"
  fi
fi
